# coding: utf-8

"""
    Subscriptions

    A subscription enables you to charge customers at specified intervals for a plan of their choice.

    The version of the OpenAPI document: 1.0.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from ls_zoho_billing_subscription.models.add_charge_response import AddChargeResponse

class TestAddChargeResponse(unittest.TestCase):
    """AddChargeResponse unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> AddChargeResponse:
        """Test AddChargeResponse
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `AddChargeResponse`
        """
        model = AddChargeResponse()
        if include_optional:
            return AddChargeResponse(
                code = 0,
                message = 'One time charge has been added successfully.',
                invoice = ls_zoho_billing_subscription.models.add_charge_response_invoice.add_charge_response_invoice(
                    invoice_id = '9030000079426', 
                    number = 'INV-384', 
                    status = 'live', 
                    invoice_items = [
                        ls_zoho_billing_subscription.models.add_charge_response_invoice_invoice_items_inner.add_charge_response_invoice_invoice_items_inner(
                            item_id = '90300000079434', 
                            name = One-time charge, 
                            description = Charges for Additional usage, 
                            code = 'basic-monthly', 
                            price = 50, 
                            quantity = 1, 
                            tags = [
                                ls_zoho_billing_subscription.models.tags_inner.tags_inner(
                                    tag_id = 56, 
                                    tag_option_id = 56, )
                                ], 
                            item_custom_fields = [
                                ls_zoho_billing_subscription.models.item_custom_fields_inner.item_custom_fields_inner(
                                    label = '', 
                                    value = '', )
                                ], 
                            item_total = 50, 
                            account_id = '90300000791111', 
                            account_name = 'General Income', )
                        ], 
                    invoice_date = '2016-06-15', 
                    due_date = '2016-06-15', 
                    payment_expected_date = '10-05-2016', 
                    ach_payment_initiated = True, 
                    transaction_type = 'invoice', 
                    customer_id = '903000000000099', 
                    customer_name = 'Benjamin', 
                    email = 'benjamin.george@bowmanfurniture.com', 
                    total = 50, 
                    payment_made = 50, 
                    balance = 0, 
                    credits_applied = 0, 
                    write_off_amount = 0, 
                    payments = [
                        ls_zoho_billing_subscription.models.buy_one_time_addon_response_invoice_payments_inner.buy_one_time_addon_response_invoice_payments_inner(
                            payment_id = '90300000079467', 
                            payment_mode = 'autotransaction', 
                            invoice_payment_id = '9030000079469', 
                            gateway_transaction_id = '9030000056453', 
                            description = '', 
                            date = '2016-06-15', 
                            reference_number = 'INV-384', 
                            amount = 50, 
                            exchange_rate = 2, )
                        ], 
                    currency_code = 'USD', 
                    currency_symbol = '$', 
                    created_time = '2016-06-11T17:57:13-0700', 
                    updated_time = '2016-06-04T16:44:15-0700', 
                    billing_address = ls_zoho_billing_subscription.models.billing_address.billing_address(
                        attention = 'Benjamin George', 
                        street = 'Harrington Bay Street', 
                        city = 'Salt Lake City', 
                        state = 'CA', 
                        country = 'U.S.A', 
                        zip = '92612', 
                        fax = '4527389', ), 
                    comments = [
                        ls_zoho_billing_subscription.models.add_charge_response_invoice_comments_inner.add_charge_response_invoice_comments_inner(
                            comment_id = '90300000003133', 
                            commented_by_id = '90300000002099', 
                            commented_by = null, 
                            comment_type = 'system', 
                            time = '2016-06-11T17:57:13-0700', 
                            operation_type = 'add', 
                            transaction_id = '9030000079467', )
                        ], )
            )
        else:
            return AddChargeResponse(
        )
        """

    def testAddChargeResponse(self):
        """Test AddChargeResponse"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
