# coding: utf-8

"""
    Addons

    An addon contains additional features that are not part of the subscribed plan, but are made available to customers on purchase of the addon. There are two kinds of addons - one-time and recurring. For a one-time addon, customers pay only once at the time of subscription, whereas for a recurring addon, customers have to pay for the addon each time they pay for the plan’s subscription. An addon can be associated with one or more plans of a product.

    The version of the OpenAPI document: 1.0.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from ls_zoho_billing_addons.models.create_an_addon_request import CreateAnAddonRequest

class TestCreateAnAddonRequest(unittest.TestCase):
    """CreateAnAddonRequest unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> CreateAnAddonRequest:
        """Test CreateAnAddonRequest
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `CreateAnAddonRequest`
        """
        model = CreateAnAddonRequest()
        if include_optional:
            return CreateAnAddonRequest(
                addon_code = 'Email-basic',
                name = 'Email addon',
                unit_name = 'Email',
                pricing_scheme = volume,
                price_brackets = [
                    ls_zoho_billing_addons.models.addon_response_price_brackets_inner.addon_response_price_brackets_inner(
                        start_quantity = 1, 
                        end_quantity = 50, 
                        price = 10, )
                    ],
                type = 'recurring',
                interval_unit = 'monthly',
                tags = [
                    ls_zoho_billing_addons.models.tags_inner.tags_inner(
                        tag_id = 460000000054182, 
                        tag_option_id = 460000000054280, )
                    ],
                custom_fields = [
                    ls_zoho_billing_addons.models.custom_fields_inner.custom_fields_inner(
                        label = 'cfitem', 
                        value = 'test', )
                    ],
                applicable_to_all_plans = False,
                plans = [
                    ls_zoho_billing_addons.models.create_an_addon_request_one_of_plans_inner.create_an_addon_request_oneOf_plans_inner(
                        plan_code = 'basic-monthly', )
                    ],
                product_id = '903000000045027',
                description = 'The addon for additonal email support',
                store_markup_description = 'Mailbox Storage-100GB | User & Docs Storage-500GB | User',
                tax_id = 'no tax will be associated',
                product_type = 'goods',
                hsn_or_sac = '74191010',
                sat_item_key_code = '10128918',
                unitkey_code = 'Yh7',
                item_tax_preferences = [
                    ls_zoho_billing_addons.models.item_tax_preferences_inner.item_tax_preferences_inner(
                        tax_specification = 'intra', 
                        tax_name = 'GST', 
                        tax_percentage = 10, 
                        tax_id = 'no tax will be associated', )
                    ],
                tax_exemption_id = '903000006345',
                tax_exemption_code = 'GST FREE'
            )
        else:
            return CreateAnAddonRequest(
                addon_code = 'Email-basic',
                name = 'Email addon',
                unit_name = 'Email',
                price_brackets = [
                    ls_zoho_billing_addons.models.addon_response_price_brackets_inner.addon_response_price_brackets_inner(
                        start_quantity = 1, 
                        end_quantity = 50, 
                        price = 10, )
                    ],
                plans = [
                    ls_zoho_billing_addons.models.create_an_addon_request_one_of_plans_inner.create_an_addon_request_oneOf_plans_inner(
                        plan_code = 'basic-monthly', )
                    ],
                product_id = '903000000045027',
        )
        """

    def testCreateAnAddonRequest(self):
        """Test CreateAnAddonRequest"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
